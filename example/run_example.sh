...
# Check if PACE scripts exist
echo ""
echo -e "${YELLOW}[Step 3/4] Checking PACE workflow scripts...${NC}"
MISSING_SCRIPTS=0

for script in pace_candidate_regions.py pace_neighborhoods.py pace_predict.py pace_filter.py pace_metrics.py; do
    if [ -f "workflow/scripts/$script" ]; then
...
if [ $MISSING_SCRIPTS -eq 1 ]; then
    echo ""
    echo -e "${RED}Error: Some PACE scripts are missing!${NC}"
    echo "Please ensure all scripts are present in workflow/scripts/"
    exit 1
fi
