...
class TestData:
    ...
    def create_all(self):
        ...
        self.create_signal_predictions()
        self.create_eqtl()
        self.create_methylation_cov()
        self.create_benchmark_inputs()
        return self
    ...
    def create_signal_predictions(self):
        """Predictions table with per-signal columns for sensitivity/eQTL."""
        import numpy as np
        import pandas as pd
        rng = np.random.default_rng(42)
        n = 400
        atac = rng.random(n)
        h3k27ac = rng.random(n)
        h3k4me1 = rng.random(n)
        meth = rng.random(n)
        h3k27me3 = rng.random(n)
        contact = rng.random(n)
        signal = 0.6 * atac + 0.4 * h3k27ac - 0.3 * meth + 0.2 * contact
        validated = (signal > np.quantile(signal, 0.7)).astype(int)
        start = rng.integers(1, 90000, n)
        df = pd.DataFrame({
            'chr': ['chr1'] * n, 'start': start, 'end': start + 500,
            'name': [f'E{i}' for i in range(n)],
            'TargetGene': [f'Gene{i % 30}' for i in range(n)],
            'distance': rng.integers(1000, 50000, n),
            'contact': contact, 'ATAC_signal': atac,
            'H3K27ac_signal': h3k27ac, 'H3K4me1_signal': h3k4me1,
            'methylation_signal': meth, 'H3K27me3_signal': h3k27me3,
            'ExpressionWeight': rng.random(n),
            'ABC.Score': signal.clip(0, None), 'validated': validated,
        })
        df.to_csv(self.test_dir / "signal_predictions.tsv", sep='\t', index=False)

    def create_eqtl(self):
        import numpy as np, pandas as pd
        rng = np.random.default_rng(1)
        eq = pd.DataFrame({'chr': ['chr1'] * 150,
                           'pos': rng.integers(1, 100000, 150),
                           'pvalue': rng.random(150) * 1e-6})
        eq.to_csv(self.test_dir / "eqtl.tsv", sep='\t', index=False)

    def create_methylation_cov(self):
        import random
        random.seed(3)
        with open(self.test_dir / "sample.bismark.cov", 'w') as f:
            for pos in range(1000, 30000, 50):
                M = random.randint(0, 30)
                U = random.randint(0, 30)
                cov = M + U
                pct = 100 * M / cov if cov > 0 else 0
                f.write(f"chr1\t{pos}\t{pos}\t{pct:.1f}\t{M}\t{U}\n")

    def create_benchmark_inputs(self):
        import pandas as pd
        df = pd.read_csv(self.test_dir / "signal_predictions.tsv", sep='\t')
        df['enhancer'] = (df['chr'] + ':' + df['start'].astype(str)
                          + '-' + df['end'].astype(str))
        df['gene'] = df['TargetGene']
        df.rename(columns={'validated': 'label'})[
            ['enhancer', 'gene']].assign(label=df['validated']).to_csv(
            self.test_dir / "bench_labels.tsv", sep='\t', index=False)
        df[['enhancer', 'gene']].assign(score=df['ABC.Score']).to_csv(
            self.test_dir / "bench_pace.tsv", sep='\t', index=False)
        df[['enhancer', 'gene']].assign(
            score=df['ABC.Score'] * 0.8).to_csv(
            self.test_dir / "bench_abc.tsv", sep='\t', index=False)
...
def run_new_module_tests(test_dir):
    """Test the modules added during the major revision."""
    print("\n" + "=" * 60)
    print("测试新增模块 (real signals, sensitivity, eQTL, benchmark)")
    print("=" * 60)

    import subprocess
    import numpy as np
    import pandas as pd
    scripts = PACE_ROOT / "scripts"

    # --- Real (non-random) signal quantification ---------------------------
    @test("multiomics: real deterministic signal quantification")
    def test_real_signal():
        import multiomics_activity as ma
        regions = pd.DataFrame({
            'chr': ['chr1', 'chr2', 'chr3'],
            'start': [1000, 2000, 3000], 'end': [1500, 2500, 3500],
            'name': ['a', 'b', 'c'],
        })
        tag = str(test_dir / "test_accessibility.tagAlign")
        v1 = ma.quantify_signal_over_regions(regions, tag)
        v2 = ma.quantify_signal_over_regions(regions, tag)
        assert np.allclose(v1, v2), "Signal quantification not deterministic"
        assert (v1 >= 0).all(), "Negative signal"
        assert v1.sum() > 0, "All-zero signal over peaks"
        return v1

    @test("multiomics: weighted geometric normalized by sum of weights")
    def test_weighted_geom_norm():
        import multiomics_activity as ma
        # If all signals are constant c, normalized weighted geom mean == c.
        calc = ma.MultiOmicsActivityCalculator(
            method=ma.AggregationMethod.WEIGHTED_GEOMETRIC)
        calc._signal_data = {}
        act = calc._aggregate_signals(
            {'A': np.full(4, 2.0), 'B': np.full(4, 2.0)},
            {}, {'A': 1.5, 'B': 1.0}, {})
        assert np.allclose(act, 2.0), f"Expected ~2.0, got {act}"
        return act

    @test("multiomics: MethylationIntegrator bedGraph path (no bash-only syntax)")
    def test_methylation_integrator():
        import multiomics_activity as ma
        meth = test_dir / "meth.bedGraph"
        with open(meth, 'w') as f:
            for p in range(1000, 6000, 100):
                f.write(f"chr1\t{p}\t{p+1}\t0.6\n")
        regions = pd.DataFrame({
            'chr': ['chr1', 'chr1'], 'start': [1000, 3000],
            'end': [2000, 4000], 'name': ['a', 'b']})
        mi = ma.MethylationIntegrator(str(meth))
        v = mi.get_methylation_scores(regions)
        assert len(v) == 2 and (0 <= v).all() and (v <= 1).all(), \
            f"Bad methylation scores: {v}"
        # Empty-region guard must not crash.
        empty = pd.DataFrame({'chr': [], 'start': [], 'end': [], 'name': []})
        assert len(mi.get_methylation_scores(empty)) == 0
        return v

    @test("multiomics: duplicate region names are not collapsed")
    def test_duplicate_names():
        import multiomics_activity as ma
        tag = str(test_dir / "test_accessibility.tagAlign")
        dup = pd.DataFrame({
            'chr': ['chr1', 'chr1', 'chr2'],
            'start': [1000, 5000, 2000], 'end': [1500, 5500, 2500],
            'name': ['dup', 'dup', 'dup']})  # all same name
        v = ma.quantify_signal_over_regions(dup, tag, region_size_norm=False)
        assert len(v) == 3, "Lost regions to name collapse"
        return v

    @test("multiomics: TF repressor lowers activity vs activator")
    def test_tf_repressor():
        import multiomics_activity as ma
        calc = ma.MultiOmicsActivityCalculator(
            method=ma.AggregationMethod.WEIGHTED_GEOMETRIC)
        sig = {'ATAC': np.array([1.0, 1.0, 1.0])}
        w = {'ATAC': 1.5}
        base = calc._aggregate_signals(sig, {}, w, {})
        inhibited = calc._aggregate_signals(
            sig, {'TF': np.array([0.0, 0.5, 1.0])}, w, {'TF': 0.5})
        assert (inhibited <= base + 1e-9).all(), "Repressor did not reduce activity"
        return inhibited

    test_real_signal()
    test_weighted_geom_norm()
    test_methylation_integrator()
    test_duplicate_names()
    test_tf_repressor()

    # --- Methylation processing -------------------------------------------
    @test("process_methylation.py: per-region beta")
    def test_methylation():
        regions = test_dir / "meth_regions.bed"
        with open(regions, 'w') as f:
            f.write("chr1\t1000\t2000\tr1\nchr1\t5000\t6000\tr2\n")
        out = test_dir / "region_meth.bedGraph"
        cmd = [sys.executable, str(scripts / "process_methylation.py"),
               "--input", str(test_dir / "sample.bismark.cov"),
               "--format", "bismark_cov", "--regions", str(regions),
               "--output", str(out), "--min_coverage", "4"]
        r = subprocess.run(cmd, capture_output=True, text=True)
        assert r.returncode == 0, f"Failed: {r.stderr}"
        df = pd.read_csv(out, sep='\t', header=None)
        assert (df[3].between(0, 1)).all(), "Methylation beta not in [0,1]"
        return df

    test_methylation()

    # --- Sensitivity analysis (with inhibitory weights + Jaccard) ----------
    @test("sensitivity_analysis.py: Jaccard + inhibitory weights")
    def test_sensitivity():
        out = test_dir / "sens.tsv"
        cmd = [sys.executable, str(scripts / "sensitivity_analysis.py"),
               "--predictions", str(test_dir / "signal_predictions.tsv"),
               "--output", str(out), "--validated_col", "validated"]
        r = subprocess.run(cmd, capture_output=True, text=True)
        assert r.returncode == 0, f"Failed: {r.stderr}"
        df = pd.read_csv(out, sep='\t')
        assert 'jaccard_top' in df.columns, "No Jaccard column"
        # Inhibitory weights must be included in the analysis.
        assert (df['type'] == 'inhibitory').any(), "No inhibitory perturbation"
        assert df['jaccard_top'].between(0, 1).all()
        return df

    test_sensitivity()

    # --- eQTL enrichment with contingency tables --------------------------
    @test("eqtl_enrichment.py: contingency table + odds ratio")
    def test_eqtl_enrichment():
        out = test_dir / "enrich.tsv"
        cmd = [sys.executable, str(scripts / "eqtl_enrichment.py"),
               "--predictions", str(test_dir / "signal_predictions.tsv"),
               "--eqtl", str(test_dir / "eqtl.tsv"),
               "--chrom_sizes", str(test_dir / "chrom.sizes"),
               "--output", str(out), "--score_col", "ABC.Score",
               "--pvalue_col", "pvalue"]
        r = subprocess.run(cmd, capture_output=True, text=True)
        assert r.returncode == 0, f"Failed: {r.stderr}"
        df = pd.read_csv(out, sep='\t')
        for col in ['pred_with_eqtl', 'pred_without_eqtl',
                    'ctrl_with_eqtl', 'ctrl_without_eqtl', 'odds_ratio']:
            assert col in df.columns, f"Missing column {col}"
        return df

    test_eqtl_enrichment()

    # --- Benchmark harness -------------------------------------------------
    @test("benchmark_compare.py: multi-method comparison")
    def test_benchmark():
        out = test_dir / "bench"
        cmd = [sys.executable, str(scripts / "benchmark_compare.py"),
               "--labels", str(test_dir / "bench_labels.tsv"),
               "--method", f"PACE:{test_dir / 'bench_pace.tsv'}:score",
               "--method", f"ABC:{test_dir / 'bench_abc.tsv'}:score",
               "--output", str(out)]
        r = subprocess.run(cmd, capture_output=True, text=True)
        assert r.returncode == 0, f"Failed: {r.stderr}"
        df = pd.read_csv(out / "benchmark_summary.tsv", sep='\t')
        assert 'auroc' in df.columns and len(df) == 2
        return df
    ...
    def test_ml_importance():
        try:
            ...
            return None
        model = test_dir / "imp_model.pkl"
        train = subprocess.run(
            [sys.executable, str(scripts / "pace_ml.py"), "train",
             "--predictions", str(test_dir / "test_predictions.tsv"),
             "--validation", str(test_dir / "test_validation.tsv"),
             "--output", str(model), "--n_estimators", "10",
             "--max_depth", "2", "--cv_folds", "3"],
            capture_output=True, text=True)
        assert train.returncode == 0, f"train failed: {train.stderr}"
        r = subprocess.run(
            [sys.executable, str(scripts / "pace_ml.py"), "importance",
             "--model", str(model), "--output", str(test_dir / "imp")],
            capture_output=True, text=True)
