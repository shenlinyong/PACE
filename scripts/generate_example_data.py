...
def generate_narrowpeak(peaks, output_file, extend=250):
    """Write the simulated peaks as a MACS2-style narrowPeak file.

    This lets the example pipeline run without MACS2/Snakemake (the candidate
    region step can consume this file directly), so the tutorial works in any
    environment with only Python + bedtools.
    """
    print(f"Generating narrowPeak: {output_file}")
    rows = []
    for i, peak in enumerate(peaks):
        center = peak['center']
        start = max(0, center - extend)
        end = min(CHROM_LENGTH, center + extend)
        strength = peak['strength']
        score = int(strength * 1000)
        signal = round(strength * 20, 3)
        summit = center - start  # peak summit offset within the region
        rows.append((CHROM_NAME, start, end, f"peak_{i}", score, '.',
                     signal, round(strength * 10, 3), round(strength * 8, 3),
                     summit))
    rows.sort(key=lambda x: x[1])
    with open(output_file, 'w') as f:
        for r in rows:
            f.write('\t'.join(str(x) for x in r) + '\n')
    print(f"  Created {len(rows)} peaks")
...
def main():
    ...
    # Generate a narrowPeak so the pipeline can run without MACS2
    generate_narrowpeak(peaks, f"{OUTPUT_DIR}/data/example_peaks.narrowPeak")
