...
def feature_importance_command(args):
    """Export feature importance for a trained model.
    ...
    gain_file = f"{out_prefix}.gain_importance.tsv"
    gain.to_csv(gain_file, sep='\t', index=False)
    logger.info("Wrote gain importance to %s", gain_file)

    plot_value, plot_err, plot_src = 'importance', None, gain

    # 2) Permutation importance (needs labelled data)
    if args.predictions and args.validation:
        logger.info("Computing permutation importance from held-out data...")
        predictions = load_predictions(args.predictions)
        validation = load_validation(args.validation,
                                     enhancer_col=args.enhancer_column,
                                     gene_col=args.gene_column)
        data = merge_with_validation(predictions, validation)
        data = FeatureEngineering.create_features(data)
        X, _ = model.prepare_features(data, feature_cols)
        y = data['validated'].values
        perm = model.permutation_feature_importance(X, y, n_repeats=args.n_repeats)
        perm_file = f"{out_prefix}.permutation_importance.tsv"
        perm.to_csv(perm_file, sep='\t', index=False)
    ...
    plot_feature_importance(plot_src, plot_file, value_col=plot_value,
                            err_col=plot_err,
                            title="PACE ML feature importance")
    logger.info("Wrote importance plot to %s", plot_file)
...
Examples:
  ...
  python scripts/pace_ml.py predict \\
    ...
    imp_parser.add_argument('--output', '-o', required=True,
                            help='Output prefix for importance files')
    imp_parser.add_argument('--predictions', default=None,
                            help='Predictions file (for permutation importance)')
    imp_parser.add_argument('--validation', default=None,
                            help='Validation pairs (for permutation importance)')
    imp_parser.add_argument('--enhancer_column', default='enhancer',
                            help='Enhancer column in validation file')
    imp_parser.add_argument('--gene_column', default='gene',
                            help='Gene column in validation file')
    imp_parser.add_argument('--n_repeats', type=int, default=20,
                            help='Permutation repeats (default: 20)')
