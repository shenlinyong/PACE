> Historical PACE documentation. These options are not the current primary model. The expression-weighted score is defined by applying the expression weight after normalization.
...
| 📚 [Full Tutorial](docs/TUTORIAL.md) | End-to-end walkthrough with I/O at every step |
| 📁 [I/O Formats](docs/IO_FORMATS.md) | Input/output file format specification |
| 📊 [Methods & Algorithm](docs/METHODS.md) | Detailed methods and formulas |
| ⚙️ [Parameters](docs/PARAMETERS.md) | Biological rationale, statistical basis & reproducibility of every default |
| 🤖 [ML Integration](docs/ML_INTEGRATION.md) | Optional ML module (matched-validation only) |
...
```bash
# Recommended: dependency-light runner (Python + bedtools only, no Snakemake/MACS2)
bash example/run_example_direct.sh

# Or, with a full environment (Snakemake + MACS2):
bash example/run_example.sh
```

This will:
1. Generate synthetic test data (5 Mb genome, 50 genes, 240 peaks)
2. Prepare reference files
3. Run the complete PACE pipeline (~12,000 example predictions)
...
| H3K9me3 | Heterochromatin | 0.5 (inhibitory) |
| TF binding | Transcription factor | 0.3 (activator *or* repressor via `TF_modes`) |

> All defaults are biologically motivated and data-informed; see [docs/PARAMETERS.md](docs/PARAMETERS.md) for the rationale, statistical basis, and how to re-derive or stress-test them.
...
PACE ships with standalone tools for reproducible analysis and validation:

| Tool | Purpose |
|------|---------|
| `scripts/process_methylation.py` | WGBS/RRBS → per-region methylation track (inhibitory signal) |
| `scripts/eqtl_enrichment.py` | eQTL enrichment with full 2×2 contingency tables, odds ratios + CI, score strata |
| `scripts/sensitivity_analysis.py` | ±50% weight perturbation; top-prediction Jaccard + AUROC stability (incl. inhibitory weights) |
| `scripts/benchmark_compare.py` | Compare PACE vs ABC / EPIPDLF / GATv2EPI / … on a common gold set |
| `scripts/pace_ml.py importance` | Feature importance (gain + permutation) and manual-vs-learned weight comparison |

See [docs/TUTORIAL.md](docs/TUTORIAL.md) for usage of each.

## Machine Learning Integration (Optional)

PACE includes an optional ML module that learns feature combinations from
validated E-G pairs. **Use it only when reliable species-matched or
context-matched validated E-P data are available** (e.g. tissue-matched eQTL or
CRISPRi). The formula-based score is the default; PACE does not transfer
human-trained models to livestock for production predictions.
...
- You have **species-/context-matched** validated E-G pairs (tissue-matched eQTL, CRISPRi, etc.)
...
Do **not** use a model trained in one species/context to make production
predictions in another; for that, use the formula-based score.
...
**Note**: the ML module is optional and requires scikit-learn (`pip install
scikit-learn`) and species-/context-matched validated pairs. The formula-based
score remains the default and needs no training data.
