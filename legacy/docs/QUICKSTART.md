...
# Run example pipeline (~1-2 minutes, Python + bedtools only)
bash example/run_example_direct.sh

# Or, with a full Snakemake + MACS2 environment:
bash example/run_example.sh
```

Results will be in `example/results/Example_Sample/` (~12,000 example predictions).

> For a complete, step-by-step walkthrough with input/output documentation,
> see [TUTORIAL.md](TUTORIAL.md). For the rationale behind every parameter, see
