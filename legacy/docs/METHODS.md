...
A = ( ∏ᵢ Sᵢ^wᵢ )^(1 / Σᵢ wᵢ)
```
The `1/Σwᵢ` normalization makes this a genuine weighted geometric **mean**, so
the activity scale is independent of the absolute weight magnitudes (doubling
all weights leaves the ranking unchanged). This is implemented identically in
`scripts/multiomics_activity.py` and `workflow/scripts/neighborhoods.py`.
...
Combines repressive marks and DNA methylation. Each inhibitory signal is
scaled to [0, 1] before weighting, so a weight is the maximum fractional
reduction of activity that signal can impose:

```
I(E) = clip( α·m̂(E) + β·Ĥ3K27me3(E) + γ·Ĥ3K9me3(E),  0, 1 )
```

Transcription factors may also contribute to `I(E)` when declared as
repressors (TFs are not assumed to be activators; see PARAMETERS.md §3).
...
| TF binding | Activator **or repressor** | 0.3 | Regulatory proteins; sign set per TF (`TF_modes`) |

> Full biological + statistical justification and the provenance of every
> default is in [PARAMETERS.md](PARAMETERS.md). DNA methylation is pre-processed
> with `scripts/process_methylation.py` (WGBS/RRBS). Robustness of the defaults
> is assessed with `scripts/sensitivity_analysis.py` (±50% perturbation,
> top-prediction Jaccard + AUROC).
