...
def safe_divide(numerator: Union[float, np.ndarray],
    ...
    elif hasattr(denominator, '__iter__') and not isinstance(denominator, str):
        # Handle other iterables
        result = np.where(np.array(denominator) != 0, 
                         np.array(numerator) / np.array(denominator), default)
    else:
        # Handle scalar
