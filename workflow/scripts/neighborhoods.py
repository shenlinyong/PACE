...
def run_neighborhoods(candidate_regions_file: str,
    ...
    # Try to read genes file, auto-detect if it has a header
    # First, peek at the file to check if first row looks like header
    with open(genes_file) as f:
        first_line = f.readline().strip().split('\t')
    
    # If first column is 'chr' or similar, it's a header
    has_header = first_line[0].lower() in ['chr', 'chrom', 'chromosome', '#chr', '#chrom', 'gene_id', 'geneid']
    
    if has_header:
        genes = pd.read_csv(genes_file, sep='\t')
    else:
        # Standard BED6 format
