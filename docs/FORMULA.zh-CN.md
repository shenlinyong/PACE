...
| 你已有的证据 | 使用的分支 | 必要数据 |
|---|---|---|
| 实测活性和启动子接触 | measured | 固定的 ATAC/DNase/H3K27ac 组合，以及实测接触或明确适用的接触先验 |
| 实测数据加序列预测 | hybrid | measured 的输入，加匹配的序列模型；做融合还需要匹配的校准器 |
| 序列或个体基因组预测 | genome | 匹配的序列模型、参考/个体基因组数据和接触先验 |

公式不要求每个检测层或每个重复单独生成一个最终文件。规范化的活性表和接触表可以包含多行，用 `sample_id` 区分每个生物学或技术重复，用 `assay` 区分检测层。PACE 会在构建分数前先处理技术重复并保留供体身份；生物学重复可以是 1、2、3 个或更多，只要样本元数据、单位和归一化协议明确。

RNA-seq、H3K27ac 以外的组蛋白标记、CTCF 和 WGBS/RRBS 默认不会成为下面公式中的额外乘法因子，而是保留为命名注释，或进入单独验证的分类器。接口见[多组学说明](MULTIOMICS.md)。
